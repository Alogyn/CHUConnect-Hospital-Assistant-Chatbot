...
const CreateService = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      AxiosInstance.post(`api/services/`, values)
        .then(() => {
          setMessage(<MyMessage messageText="Service créé avec succès !" messagecolor="green" />);
    ...
    <div>
      <form onSubmit={formik.handleSubmit}>
        <Box className={"TopBar"}>
          <AddBoxIcon />
          <Typography sx={{ marginLeft: '15px', fontWeight: 'bold' }} variant='subtitle2'>Créer un nouveau service</Typography>
        </Box>
        {message}
